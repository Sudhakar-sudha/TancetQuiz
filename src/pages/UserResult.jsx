...
const UserResult = () => {
  ...
  useEffect(() => {
    fetchStudents();
    const interval = setInterval(fetchStudents, 5000); // Auto-update every 5 seconds
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      <h1 className="text-2xl font-bold text-center mb-4">📋 Student Results (Auto-Update)</h1>
      ...
      <div className="overflow-x-auto mt-6">
        <table className="min-w-full bg-white border border-gray-200 shadow-md">
          ...
          <tbody>
            {students.length > 0 ? (
              students.map((student) => (
                <tr key={student._id} className="text-center border-b hover:bg-gray-100 transition">
                  <td className="py-2 px-4 border">{student.username}</td>
                  <td className="py-2 px-4 border">{student.totalScore}</td>
                  <td className="py-2 px-4 border">{student.totalQuestions}</td>
                  <td className="py-2 px-4 border">{student.correctAnswers}</td>
